...
def workers_sentiment_detailed_results(request):
    ...
    for i in range(1,6):
        ...
        answers_for_part = workers_sentiment_qs.filter(question__questionset__sortid=i).order_by('question__sort_id')
        ...
        for idx2, runid in enumerate(runids):
            answers_for_part_for_user = answers_for_part.filter(runid=runid).order_by('question__sort_id')
